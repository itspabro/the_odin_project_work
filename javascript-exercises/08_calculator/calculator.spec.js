...
describe('sum', () => {
  ...
  test('computes the sum of multiple arrays of many numbers', () => {
    expect(calculator.sum([1, 2, 3], [4, 5], [6, [7, [8, 9]]])).toBe(45);
  });

  test('returns ERROR when fed NaN', () => {
    expect(calculator.sum([1, 2, 3], [4, 'F'], [6, [7, [8, 9]]])).toBe("ERROR");
  });
});
...
describe('multiply', () => {
  ...
  test('multiplies several numbers including 0', () => {
    expect(calculator.multiply([2, 4, 6, 8, 10, 12, 0, 14])).toBe(0);
  });
});
...
describe('power', () => {
  ...
  test('raises one number to the zero power', () => {
    expect(calculator.power(3, 0)).toBe(1); // 3 to tenth power is 59049
  });
});
